...
@section('add_module')
	...
	<div class='container'>
		<div id='add_module' class='col-md-4 col-md-offset-4 form_wrapper m-form'>		
			...
			<h5 class='heading'>{{Lang::get('caption.title.add_module')}}</h5>			 	
		 	<div class='input-group'>		 	 	  
				<span class='place'>*</span>
				<span class='input-group-addon'><span class='glyphicon glyphicon-list-alt'></span></span>					
				{{ Form::hidden('projectId', Session::get('projectId'), 
					array('class' => 'form-control', 
						  'id'	  => 'txtProjectId'))
				}}
				{{ Form::text('modulename', Input::old('modulename'),
					array('class' 	 		=> 'form-control', 
						   'placeholder'	=> Lang::get('caption.label.add_module.modulename'))) 
				}}
			</div>
			<p class='error_msg'>
				@if($errors->has('modulename'))
					{{ $errors->get('modulename')[0];}}						
				@endif
			</p>
			...
			<div class='input-group'>
				...
				{{ Form::text('desc', Input::old('desc')
					,array('class'  		=> 'form-control', 
						   'placeholder'	=> Lang::get('caption.label.add_module.desc'))) 
				}}
			</div>		
			...
			<div class='input-group'>
				  ...
				  {{ Form::text('startDate',Input::old('startDate'),
				  		array('class'			 => 'form-control date_picker',
				  			  ...
				  			  'placeholder' 	 => Lang::get('caption.label.add_module.start_date'),
			...
			<div class='input-group'>
				<span class='place'>*</span>
				    {{ Form::text('dueDate',Input::old('dueDate'),
						array('class'		  	=> 'form-control date_picker', 
							'data-date-format'	=> 'dd-MM-yyyy', 
							'placeholder'		=> Lang::get('caption.label.add_module.due_date'),
							'id'	 		 	=> 'DueDate', 'readonly'))
					}}
				<span class='input-group-addon'><i class='glyphicon glyphicon-calendar'></i></span>
			</div>
			...
			<div id='btn' class='col-md-12'>		    	
				<p>
					{{ Form::button('Create', array('class' => 'btn btn-default', 'id' => 'btnSubmit')) }}			
				</p>
		    </div>		    
		{{Form::close()}}
